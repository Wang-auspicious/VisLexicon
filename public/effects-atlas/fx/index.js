...
export const CATEGORY_FILES = [
  'font', 'text', 'type', 'container', 'entrance', 'transition', 'easing', 'physics',
...
export const GROUPS = [
  ...
  { id: 'type', zh: '文字与排版', en: 'Type & Typography', of: ['type-study', 'font', 'type', 'text'] },
  ...
  { id: 'struct', zh: '结构与信息', en: 'Structure', of: ['composition-study', 'layout', 'radial', 'list', 'nav', 'overlay', 'loading', 'table', 'gui'] },
  { id: 'material', zh: '材质与光色', en: 'Material & light', of: ['light-study', 'space-study', 'background', 'gradient', 'filter', 'mask', 'shadow', 'threed'] },
  { id: 'craft', zh: '图形与媒体', en: 'Graphics & media', of: ['svg', 'media', 'particle', 'marquee', 'number', 'dataviz', 'ascii'] },
  { id: 'basics', zh: '视觉基本功', en: 'Visual fundamentals', of: ['grid-study', 'color', 'grid', 'icon', 'photo'] },
...
export async function loadCategories() {
  ...
  const categories = mods.filter(Boolean);
  const { addPractices } = await import('../practices.js');
  return addPractices(categories);
}
...
export function composePrompt(item, cat, lang, params, motion) {
  if (item.practice) return (item.practice.prompt && (item.practice.prompt[lang] || item.practice.prompt.zh)) || '';
