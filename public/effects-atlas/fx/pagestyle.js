...
.fxcta{background:#e8574c;border-radius:999px}`,
    ...
    I("dithermono", "点阵单色风", "Dithered monochrome",
      "一张抖出来的黑白点阵图当整页主角，字是等宽，颜色只有黑与白", "One dithered black-and-white image carries the page; monospace type, no colour at all",
      "把照片压成 1bit：阈值抖动或网点半调，全站只留黑白两色加一款等宽字，标题和正文靠字号拉开层级。抖动是刻意的，不是压缩失真。",
      "Reduce photography to 1-bit with order dithering or halftone, hold the palette to black and white, and let one monospace face carry display and body copy through size alone. The dither is deliberate, not a compression artefact.",
      ["1bit", "等宽", "抖动", "黑白"], ["1-BIT", "MONOSPACE", "DITHER", "BLACK & WHITE"],
      ["", "MOONBIT      Join us   About   Roles", "MAKE THE\nunmade", "我们造还没被造出来的东西。", "查看岗位", ""],
      `@keyframes fxk{0%{background-position:0 0}100%{background-position:22px 22px}}
.fxpage{background:#000;border-color:#1c1c1c}
.fxbar{background:#000;border-bottom-color:#1c1c1c}
.fxbar::after{background:#0d0d0d;border-color:#242424}
.fxnav2{background:#000;color:#fff;border-bottom-color:#1c1c1c;font-family:var(--fx-mono,ui-monospace,monospace);letter-spacing:.06em}
.fxhead{font:400 30px/1.06 var(--fx-mono,ui-monospace,monospace);color:#fff;letter-spacing:-.01em;text-transform:uppercase}
.fxsub{color:#8a8a8a;font-family:var(--fx-mono,ui-monospace,monospace);font-size:9.5px}
.fxcta{background:#fff;color:#000;border-radius:0;font-family:var(--fx-mono,ui-monospace,monospace)}
.fxart{background-color:#000;background-image:radial-gradient(#fff 1px,transparent 1.2px),radial-gradient(#fff 1px,transparent 1.2px),radial-gradient(circle at 50% 46%,#000 0 34%,#fff 34.5% 100%);background-size:11px 11px,11px 11px,100% 100%;background-position:0 0,5.5px 5.5px,0 0;animation:fxk 5s linear infinite}`,
      R(["Kimi 招聘官网", "careers.kimi.com"])),

    I("dotmatrix", "仪器点阵风", "Instrument dot-matrix",
      "一块点阵屏加一个旋钮，像硬件面板而不是网页", "A dot-matrix display and a knob — a hardware panel, not a web page",
      "主视觉用 LED 点阵屏，背景是细网格，标签一律等宽大写；按钮画成实体键，带按下位移和轻微投影。整体直角、低饱和、金属灰。",
      "Lead with an LED matrix, lay a hairline grid behind everything, set every label in uppercase mono, and draw buttons as physical keys that travel when pressed. Right angles, desaturated grey, no ornament.",
      ["点阵屏", "硬件感", "等宽", "实体键"], ["LED MATRIX", "HARDWARE", "MONOSPACE", "PHYSICAL KEY"],
      ["", "POLYFORM      Devices   Lab   Shop", "Precision\nyou can feel", "旋钮、按键、屏幕，都是真的。", "看产品", ""],
      `@keyframes fxk{0%,100%{opacity:.35}50%{opacity:1}}
.fxpage{background:#d8d8d4;border-color:#c2c2bc;box-shadow:0 1px 2px #0000001a,0 24px 48px -26px #21211c66}
.fxbar{background:#c8c8c2;border-bottom-color:#b4b4ae}
.fxbar::after{background:#e4e4df;border-color:#b4b4ae}
.fxnav2{background:#e2e2dd;color:#2b2b26;font-family:var(--fx-mono,ui-monospace,monospace);letter-spacing:.1em;text-transform:uppercase;font-size:9px}
.fxhead{font:700 26px/1.08 var(--fx-sans,system-ui);color:#21211c;letter-spacing:-.024em}
.fxsub{color:#5f5f58}
.fxcta{background:linear-gradient(#4a4a44,#2b2b26);color:#f2f2ee;border-radius:3px;box-shadow:0 2px 0 #16160f,0 6px 12px -6px #00000066;animation:fxk 3.6s ease-in-out infinite}
.fxart{background-color:#1e1e19;background-image:radial-gradient(circle,#e6c14a 1.6px,transparent 1.8px);background-size:13px 13px;box-shadow:inset 0 0 0 6px #2f2f29,inset 0 0 22px #000c}`,
      R(["Teenage Engineering", "teenage.engineering"])),

    I("streetwear", "街头大片风", "Streetwear campaign",
      "一张满幅人物大片顶在最上面，标题粗到有点过分", "A full-bleed campaign photo on top, headline heavier than it needs to be",
      "首屏就是一张满幅人物照，粗黑无衬线标题压在图上；商品用横滑一排在下面。动效只做位移和缩放，不玩花活。",
      "Open with a full-bleed photo, set the headline in the heaviest grotesque you can justify, and put products in a single horizontal rail under it. Motion is limited to translate and scale.",
      ["满幅大片", "粗黑标题", "横滑", "运动感"], ["FULL-BLEED", "HEAVY GROTESQUE", "RAIL", "ATHLETIC"],
      ["", "STRIDE      New   Men   Women   Sale", "MOVE\nWITHOUT\nPERMISSION", "为跑起来的人做的。", "立即选购", ""],
      `@keyframes fxk{0%,100%{transform:scale(1.02) translateX(0)}50%{transform:scale(1.08) translateX(-2%)}}
.fxpage{background:#ffffff;border-color:#e6e6e6}
.fxbar{background:#111;border-bottom-color:#111}
.fxbar::before{background:#555;box-shadow:9px 0 0 #555,18px 0 0 #555}
.fxbar::after{background:#2a2a2a;border-color:#3a3a3a}
.fxnav2{background:#111;color:#fff;border-bottom-color:#111;letter-spacing:.02em}
.fxhead{font:900 27px/0.94 var(--fx-sans,system-ui);color:#111;letter-spacing:-.045em;text-transform:uppercase}
.fxsub{color:#6b6b6b}
.fxcta{background:#111;color:#fff;border-radius:999px;padding:9px 20px}
.fxart{background:linear-gradient(200deg,#c9d4dc 0 42%,#5c6b76 42% 100%);position:relative}
.fxart::after{content:'';position:absolute;inset:-12% -8%;background:radial-gradient(58% 44% at 44% 38%,#e8d7c8,transparent 62%),radial-gradient(44% 60% at 70% 78%,#2f3a42,transparent 66%);animation:fxk 9s ease-in-out infinite}`,
      R(["Nike", "nike.com"])),

    I("gallery", "美术馆风", "Art museum",
      "白墙、黑字、大留白，标题大而中性，说明字小到几乎听不见", "White walls, black type, vast margins — big neutral headline, whisper-small captions",
      "底色接近纯白但不刺眼；标题用中性无衬线或过渡衬线，字距收一点。展览信息、票务、赞助人一律小字排在底部，中间只放一件作品。",
      "Near-white ground, one neutral sans or transitional serif at display size with tight tracking, and everything logistical — dates, tickets, credits — set small along the base. The middle belongs to a single work.",
      ["白墙", "大留白", "中性标题", "小字信息"], ["WHITE CUBE", "WHITE SPACE", "NEUTRAL DISPLAY", "SMALL CAPS"],
      ["", "THE HALL      Visit   Exhibitions   Collection", "The shape\nof now", "正在展出：形式的边界（9.12–12.04）", "预约参观", ""],
      `@keyframes fxk{0%,100%{opacity:.5;transform:translateY(6px)}45%,75%{opacity:1;transform:none}}
.fxpage{background:#fbfbfa;border-color:#eaeae7}
.fxbar{background:#fbfbfa;border-bottom-color:#eaeae7}
.fxbar::before{background:#dcdcd8}
.fxbar::after{background:#f2f2ef;border-color:#e4e4e0}
.fxnav2{background:#fbfbfa;color:#1b1b1b;border-bottom-color:#eaeae7;font-size:9px;letter-spacing:.12em;text-transform:uppercase}
.fxhead{font:400 30px/1.04 Georgia,'Times New Roman',serif;color:#141414;letter-spacing:-.018em}
.fxsub{color:#8b8b86;font-size:9.5px;letter-spacing:.01em}
.fxcta{background:transparent;color:#141414;border:1px solid #141414;border-radius:0}
.fxart{background:radial-gradient(58% 52% at 50% 46%,#e7e3da,#f4f3ef 62%,#fbfbfa);position:relative}
.fxart::after{content:'';position:absolute;left:30%;top:24%;width:40%;height:52%;background:linear-gradient(160deg,#2f2f2c,#6c6a63);animation:fxk 8s ease-in-out infinite}`,
      R(["MoMA", "moma.org"])),

    I("japanweb", "日式生活网页", "Japanese daily web",
      "栏目多、字号小、行距松，用细边框把每一块分开", "Many columns, small type, generous leading — every block fenced by a hairline",
      "信息密度高但不挤：正文 12–13px、行高 1.9，块与块之间只用 1px 细线分隔；配色克制到近乎无色，重点靠加粗和缩进。",
      "Dense but unhurried: 12–13px body at 1.9 leading, blocks separated only by hairlines, a palette that is almost no palette. Emphasis comes from weight and indent, never from colour.",
      ["高密度", "细边框", "小字号", "松行距"], ["DENSE", "HAIRLINE", "SMALL TYPE", "LOOSE LEADING"],
      ["", "DAILY      今日の  連載  過去  案内", "毎日の\nsmall things", "把小的东西一天天写下去。", "続きを読む", ""],
      `@keyframes fxk{0%,100%{opacity:.65}50%{opacity:1}}
.fxpage{background:#fdfdfb;border-color:#dcdcd6}
.fxbar{background:#f4f4f0;border-bottom-color:#dcdcd6}
.fxbar::after{background:#fff;border-color:#dcdcd6}
.fxnav2{background:#fafaf7;color:#2a2a26;border-bottom-color:#e2e2dc;font-size:9px;letter-spacing:.02em}
.fxhead{font:700 22px/1.36 var(--fx-sans,system-ui);color:#1f1f1c;letter-spacing:.01em}
.fxsub{color:#6f6f68;font-size:10px;line-height:1.95}
.fxcta{background:#fff;color:#1f1f1c;border:1px solid #cfcfc8;border-radius:2px}
.fxart{background:#f7f7f3;border-left:1px solid #e2e2dc;position:relative}
.fxart::after{content:'';position:absolute;inset:14px;background:repeating-linear-gradient(180deg,#e6e6df 0 1px,transparent 1px 26px);animation:fxk 6s ease-in-out infinite}`,
      R(["ほぼ日刊イトイ新聞", "1101.com"])),

    I("quietgrid", "安静网格风", "Quiet grid",
      "一列窄栏、大量留白，链接是深蓝不是亮色", "A single narrow column, a lot of margin, links in navy rather than a bright accent",
      "正文一栏不超过 60 字符，中线对齐；链接用深蓝加下划线，激活态才变色。图片小、少、方，其余交给留白。",
      "Keep the measure under 60 characters on the centre line. Links are navy with an underline and only change on activation. Images stay small, few and square; everything else is margin.",
      ["窄栏", "长留白", "深蓝链接", "方块图"], ["NARROW MEASURE", "MARGIN", "NAVY LINK", "SQUARE IMAGE"],
      ["", "INDEX      Log in   About   How it works", "A place to\nkeep things", "先把东西存下来，再慢慢长出关系。", "Sign up", ""],
      `@keyframes fxk{0%,100%{opacity:.4}48%,76%{opacity:1}}
.fxpage{background:#ffffff;border-color:#ececec}
.fxbar{background:#fafafa;border-bottom-color:#ececec}
.fxbar::after{background:#fff;border-color:#ececec}
.fxnav2{background:#fff;color:#1a1a1a;border-bottom-color:#f2f2f2;font-size:9.5px}
.fxhead{font:600 25px/1.12 var(--fx-sans,system-ui);color:#111;letter-spacing:-.022em}
.fxsub{color:#6e6e6e;max-width:30ch}
.fxcta{background:#00007a;color:#fff;border-radius:3px}
.fxart{background:linear-gradient(#f6f6f6,#efefef);border-left:1px solid #f2f2f2;position:relative}
.fxart::after{content:'';position:absolute;left:18%;top:22%;width:44%;height:44%;background:#e4e4e4;box-shadow:0 0 0 1px #dcdcdc;animation:fxk 9s ease-in-out infinite}`,
      R(["Are.na", "are.na"])),

    I("radioprint", "电台文化风", "Radio culture",
      "深底加一块高饱和色，节目单是列表，播放器常驻底部", "A dark ground, one saturated block, a schedule as a plain list, a player that never leaves",
      "背景近黑，主色只用一个高饱和色；节目按时间排成无边框列表，当前节目用色块标出。底部常驻一条播放器，字号收紧、字距拉近。",
      "Near-black ground, exactly one saturated accent. The schedule is an unfenced list ordered by time, the current slot marked by a colour block, and a player bar that stays put. Type tight, tracking tighter.",
      ["暗底", "单高饱和色", "列表节目单", "常驻播放器"], ["DARK", "ONE ACCENT", "LIST SCHEDULE", "STICKY PLAYER"],
      ["", "SIGNAL      Listen   Shows   Schedule", "ON AIR,\nall night", "全天候的电台与节目单。", "收听直播", ""],
      `@keyframes fxk{0%,100%{transform:translateY(0);opacity:.55}50%{transform:translateY(-4px);opacity:1}}
.fxpage{background:#0e0e10;border-color:#22222a}
.fxbar{background:#0e0e10;border-bottom-color:#22222a}
.fxbar::before{background:#3a3a44}
.fxbar::after{background:#16161a;border-color:#2c2c36}
.fxnav2{background:#0e0e10;color:#e8e8ee;border-bottom-color:#22222a;letter-spacing:.06em}
.fxhead{font:800 27px/1.02 var(--fx-sans,system-ui);color:#fff;letter-spacing:-.03em;text-transform:uppercase}
.fxsub{color:#9a9aa6}
.fxcta{background:#ff3b30;color:#fff;border-radius:2px;animation:fxk 3.4s ease-in-out infinite}
.fxart{background:linear-gradient(160deg,#ff3b30 0 26%,#141418 26%);position:relative}
.fxart::after{content:'';position:absolute;left:16%;bottom:14%;width:68%;height:8px;background:repeating-linear-gradient(90deg,#e8e8ee 0 3px,transparent 3px 8px);opacity:.7}`,
      R(["NTS Radio", "nts.live"])),

    I("horology", "腕表工艺风", "Watch craftsmanship",
      "近黑的底、金色细线、衬线标题，产品放正中间", "Near-black ground, gold hairlines, a serif headline and the product dead centre",
      "底色压到近黑，分隔线用 1px 香槟金；标题是衬线，正文小到 9px 并拉开字距。产品永远居中，四周全是留白。",
      "Push the ground to near-black and draw every divider in a 1px champagne gold. Serif headline, 9px body with open tracking, the product always centred and nothing crowding it.",
      ["近黑", "香槟金", "衬线标题", "居中产品"], ["NEAR-BLACK", "CHAMPAGNE GOLD", "SERIF", "CENTRED"],
      ["", "AURUM      Collection   Craft   Service", "Time,\nkept", "每一枚都要经手三十次。", "探索系列", ""],
      `@keyframes fxk{0%,100%{opacity:.35;transform:rotate(0)}50%{opacity:1;transform:rotate(6deg)}}
.fxpage{background:#0b0b0c;border-color:#26241c}
.fxbar{background:#0b0b0c;border-bottom-color:#26241c}
.fxbar::before{background:#5a5238;box-shadow:9px 0 0 #5a5238,18px 0 0 #5a5238}
.fxbar::after{background:#121213;border-color:#3a3524}
.fxnav2{background:#0b0b0c;color:#dcd6c2;border-bottom-color:#26241c;letter-spacing:.16em;text-transform:uppercase;font-size:8.5px}
.fxhead{font:400 30px/1.06 Georgia,'Times New Roman',serif;color:#f0ead6;letter-spacing:.004em}
.fxsub{color:#8d8672}
.fxcta{background:transparent;color:#d9c283;border:1px solid #6f6237;border-radius:0;letter-spacing:.14em}
.fxart{background:radial-gradient(46% 46% at 50% 48%,#1c1c1e,#0b0b0c 72%);border-left:1px solid #26241c;position:relative}
.fxart::after{content:'';position:absolute;left:33%;top:30%;width:34%;height:34%;border-radius:50%;border:2px solid #c9ab5e;box-shadow:inset 0 0 0 5px #0b0b0c,inset 0 0 0 6px #6f6237;animation:fxk 8s ease-in-out infinite}`,
      R(["Rolex", "rolex.com"])),

    I("timetable", "时刻表功能风", "Timetable functionalism",
      "表格优先，行高固定，字号极小，颜色只用来标状态", "Table first: fixed row heights, tiny type, colour used only to mark state",
      "一切对齐到列：站名、时间、站台三列，行高恒定 22px；红蓝是功能色不是装饰色。信息层级靠字重和灰阶，不靠色块。",
      "Everything snaps to columns — name, time, platform — at a fixed 22px row height. Red and blue are functional signals, never decoration. Hierarchy comes from weight and grey level, not fills.",
      ["表格优先", "固定行高", "功能色", "对齐到列"], ["TABLE FIRST", "FIXED ROW", "FUNCTIONAL COLOUR", "COLUMN SNAP"],
      ["", "LINEA      Routes   Tickets   Help", "Plan the\ntrip", "准点也是一种设计。", "查班次", ""],
      `@keyframes fxk{0%,100%{opacity:.5}50%{opacity:1}}
.fxpage{background:#fff;border-color:#d8d8d8}
.fxbar{background:#f0f0f0;border-bottom-color:#d8d8d8}
.fxbar::before{background:#c3c3c3}
.fxbar::after{background:#fff;border-color:#d8d8d8}
.fxnav2{background:#fff;color:#1a1a1a;border-bottom-color:#e4e4e4;font-size:9px;letter-spacing:.04em}
.fxhead{font:600 24px/1.16 var(--fx-sans,system-ui);color:#111;letter-spacing:-.014em}
.fxsub{color:#5a5a5a;font-size:10px}
.fxcta{background:#c8102e;color:#fff;border-radius:2px}
.fxart{background:repeating-linear-gradient(180deg,#fff 0 21px,#ededed 21px 22px);border-left:1px solid #e4e4e4;position:relative}
.fxart::after{content:'';position:absolute;left:0;top:0;bottom:0;width:44%;box-shadow:inset 3px 0 0 #0b57a4;background:linear-gradient(90deg,#0b57a40f,transparent);animation:fxk 6.5s ease-in-out infinite}`,
      R(["SBB", "sbb.ch"])),

    I("chromatic", "彩光玻璃风", "Chromatic glass",
      "半透明彩色叠在一起，圆角很大，阴影很浅", "Translucent colour layers stacked, generous rounding, barely-there shadow",
      "彩色层用 40%–70% 透明叠加，背景是柔和渐变；圆角 18–22px，阴影浅到几乎看不见。文字压在玻璃上，靠字重而不是描边保证可读。",
      "Stack colour layers at 40–70% opacity over a soft gradient, round corners to 18–22px, and keep shadows almost invisible. Type sits on the glass and stays readable through weight, not outlines.",
      ["半透明", "大圆角", "浅阴影", "柔和渐变"], ["TRANSLUCENT", "BIG RADIUS", "FAINT SHADOW", "SOFT GRADIENT"],
      ["", "PRISM      Browse   Spaces   Notes", "The web,\nrecoloured", "把网页重新上色。", "下载", ""],
      `@keyframes fxk{0%,100%{transform:translate3d(-4%,-2%,0) scale(1.04)}50%{transform:translate3d(5%,3%,0) scale(1.12)}}
.fxpage{background:linear-gradient(140deg,#fdf4ff,#eef4ff 52%,#f2fff8);border-color:#e6e6f2;border-radius:20px}
.fxbar{background:#ffffff8c;border-bottom-color:#e2e2ef}
.fxbar::after{background:#ffffffb0;border-color:#dedeec}
.fxnav2{background:#ffffff66;color:#2a2b3d;border-bottom-color:#e2e2ef}
.fxhead{font:700 26px/1.08 var(--fx-sans,system-ui);color:#232438;letter-spacing:-.028em}
.fxsub{color:#5d5f78}
.fxcta{background:#4b3bff;color:#fff;border-radius:999px}
.fxart{background:radial-gradient(52% 48% at 32% 34%,#ff8fd066,transparent 62%),radial-gradient(56% 52% at 72% 62%,#7fd8ff66,transparent 64%),linear-gradient(150deg,#efe6ff,#e4f4ff);position:relative}
.fxart::after{content:'';position:absolute;left:22%;top:20%;width:52%;height:58%;border-radius:24px;background:#ffffff3d;border:1px solid #ffffff8a;backdrop-filter:blur(6px);animation:fxk 11s ease-in-out infinite}`,
      R(["Arc browser", "arc.net"])),

    I("botanical", "植物图鉴风", "Botanical field guide",
      "米白纸底、手绘标本、拉丁学名用小字排在下面", "Cream paper, drawn specimens, Latin binomials set small beneath",
      "底色偏米黄像旧纸；插画用细线平涂，不描边不渐变。每张卡下方是中文学名 + 拉丁双名 + 采集地，字小但整齐。",
      "A cream ground that reads as old paper, plates drawn in flat fills with hairline outlines, and each card footed by local name, Latin binomial and locality in small orderly type.",
      ["米白纸", "标本插画", "拉丁学名", "细线"], ["CREAM PAPER", "PLATE", "LATIN NAME", "HAIRLINE"],
      ["", "HERBARIUM      Plants   Learn   Visit", "Field\nnotes", "每一株都记下名字和季节。", "翻图鉴", ""],
      `@keyframes fxk{0%,100%{transform:rotate(-2deg)}50%{transform:rotate(2deg)}}
.fxpage{background:#f7f2e4;border-color:#e2d9c3}
.fxbar{background:#efe8d6;border-bottom-color:#e2d9c3}
.fxbar::after{background:#fbf8ee;border-color:#e2d9c3}
.fxnav2{background:#f2ecdc;color:#3d3a2c;border-bottom-color:#e2d9c3;letter-spacing:.1em;text-transform:uppercase;font-size:8.5px}
.fxhead{font:400 27px/1.1 Georgia,'Times New Roman',serif;color:#2f3a24;letter-spacing:-.01em}
.fxsub{color:#6b6552;font-size:9.5px}
.fxcta{background:#3f4d2c;color:#f7f2e4;border-radius:2px}
.fxart{background:radial-gradient(58% 54% at 50% 50%,#efe8d6,#f7f2e4 72%);border-left:1px solid #e2d9c3;position:relative}
.fxart::after{content:'';position:absolute;left:34%;top:16%;width:32%;height:68%;border-radius:50% 50% 42% 42%;background:linear-gradient(#5c7a3a,#3f4d2c);box-shadow:inset 0 0 0 1px #2f3a24;animation:fxk 9s ease-in-out infinite}`,
      R(["Kew Gardens", "kew.org"])),

    I("typefoundry", "字体铸造厂风", "Type foundry",
      "页面本身就是一张字体样本：大字试样、极窄行距、等宽小字标注", "The page is the specimen: giant glyphs, razor-tight leading, mono metadata",
      "主视觉直接排大字试样，行距压到 0.86；字重、字号、字距用等宽小字标在旁边。不放大图，页面说的一切都由字承担。",
      "Set the specimen itself as the hero at 0.86 leading, and annotate weight, size and tracking in mono down the side. No imagery: the type carries every claim on the page.",
      ["字体试样", "窄行距", "等宽标注", "无图"], ["SPECIMEN", "TIGHT LEADING", "MONO METADATA", "NO IMAGERY"],
      ["", "GROTESK      Specimens   Licences   About", "Aa\nBb\nRr", "一套字，二十四种字重。", "试排", ""],
      `@keyframes fxk{0%,100%{font-weight:300;letter-spacing:-.01em}50%{font-weight:800;letter-spacing:-.045em}}
.fxpage{background:#fff;border-color:#e0e0e0}
.fxbar{background:#f4f4f4;border-bottom-color:#e0e0e0}
.fxbar::after{background:#fff;border-color:#e0e0e0}
.fxnav2{background:#fff;color:#141414;border-bottom-color:#ececec;font-family:var(--fx-mono,ui-monospace,monospace);font-size:9px;letter-spacing:.08em}
.fxhead{font:600 34px/0.86 var(--fx-sans,system-ui);color:#0d0d0d;letter-spacing:-.04em;white-space:pre-line;animation:fxk 7s ease-in-out infinite}
.fxsub{color:#7a7a7a;font-family:var(--fx-mono,ui-monospace,monospace);font-size:9px}
.fxcta{background:#0d0d0d;color:#fff;border-radius:0;font-family:var(--fx-mono,ui-monospace,monospace)}
.fxart{background:#fff;border-left:1px solid #ececec;position:relative}
.fxart::after{content:'';position:absolute;inset:0;background:repeating-linear-gradient(180deg,#f0f0f0 0 1px,transparent 1px 18px),repeating-linear-gradient(90deg,#f0f0f0 0 1px,transparent 1px 18px)}`,
      R(["Klim Type Foundry", "klim.co.nz"])),
