...
test('private draft buttons stay out of public search',()=>{
  ...
  for(const response of [null,{mode:'empty',rows:[]}]) {
    ...
    assert.deepEqual(rows.map(row=>row.unit.id),[])
  }
  assert.equal(discoveryResults('button',[site],{rows:[]},{scope:'curation',theme:'ui-implementation',drafts:[draft]}).length,0)
