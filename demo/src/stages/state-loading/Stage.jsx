  ...
  return (
    <div className={`sl sl-${preset}`} style={style}>
      <div className="sl-head">
        ...
        <span {...node('state.conn', 'sl-conn')}><i />已连接</span>
      </div>

      {preset === 'loading' && (
        <div {...node('state.skeleton', 'sl-skeleton')} key={`sk-${replayKey}`}>
          ...
          <div {...node('state.shimmer', 'sl-shimmer')} aria-hidden="true" />
        </div>
      )}
        ...
        <div className="sl-rows">
          {['构建 #482', '构建 #481', '构建 #480'].map((r, i) => (
            <div key={r} className="sl-row">
              <span>{r}</span>
              {i === 0
                ? <span {...node('state.loader', 'sl-loader')} aria-label="进行中"><i /></span>
                : <span className="sl-ok">成功</span>}
              {i === 0
                ? <span {...node('state.ticker', 'sl-ticker')}>{n.toLocaleString()}</span>
                : <span className="sl-plain">318</span>}
            </div>
          ))}

          <div {...node('state.job', 'sl-job')}>
