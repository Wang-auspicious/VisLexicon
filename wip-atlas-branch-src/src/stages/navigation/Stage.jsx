...
export default function NavigationStage({ stage, variant, values, activeNode, hoverNode, onHover }) {
  ...
  return (
    <div className="nv" style={style}>
      <span {...node('nav.skip', 'nv-skip')} data-zone="service-level">跳转到主内容</span>

      <div {...node('nav.service', 'nv-service')} data-zone="service-level">
        <b>视元</b>
        <span>控制台</span>
        <span>文档</span>
        <span className="nv-spacer" />
        <span {...node('nav.lang', 'nv-lang')} data-zone="service-level">中文 / EN</span>
      </div>

      <div className="nv-body">
        <aside className="nv-rail">
          <div {...node('nav.menubar', 'nv-menubar')} data-zone="app-level">
            <i className="on">概览</i><i>运行</i><i>成员</i><i>设置</i>
          </div>
          <div {...node('nav.anchor', 'nv-anchor')} data-zone="within-page">
            <em>本页</em><i>· 基本信息</i><i>· 配额</i><i>· 危险操作</i>
          </div>
        </aside>

        <main className="nv-main">
          <div {...node('nav.breadcrumb', 'nv-breadcrumb')} data-zone="page-level">
            <span>组织</span><i>/</i><span>视元</span><i>/</i><span>运行</span>
          </div>

          <div className="nv-toprow">
            <span {...node('nav.back', 'nv-back')} data-zone="page-level">← 返回运行列表</span>
            <span className="nv-spacer" />
            <div {...node('nav.toolbar', 'nv-toolbar')} data-zone="app-level">
              <i>⟳</i><i>⤓</i><i>⧉</i>
            </div>
            <span {...node('nav.menubtn', 'nv-menubtn')} data-zone="app-level">更多 ⌄</span>
            <div {...node('nav.menu', 'nv-menu')} data-zone="app-level">
              <i>重新运行</i><i>下载日志</i><i className="nv-danger">删除</i>
            </div>
          </div>

          <div {...node('nav.tabs', 'nv-tabs')} data-zone="page-level">
          ...
          <div {...node('nav.pagination', 'nv-pagination')} data-zone="within-page">
            <i>‹</i><i className="on">1</i><i>2</i><i>3</i><i>…</i><i>9</i><i>›</i>
          </div>
        </main>
      </div>

      <span {...node('nav.scrollbtn', 'nv-scrollbtn')} data-zone="within-page" aria-label="回到顶部">↑</span>
