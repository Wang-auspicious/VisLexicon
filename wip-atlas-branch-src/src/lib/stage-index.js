 ...
 *
 * 每台还声明三件事（方案 §3.2 / §3.3 / §5.2），一并在这里校验：
 *   zones[]            分区，每区 3–7 条，全台术语必须被且只被一个分区覆盖
 *   compareSets[]      对照组，一张行为矩阵，成员必须是本台认领过的术语
 *   positionRegions[]  这台的术语落在七个页面位置区域里的哪几个
 */
import {
  COMPARE_AXES,
  POSITION_REGION_IDS,
...
/* 分区 / 对照组 / 位置区域的校验。
 * 这三样都是「写错了界面上什么都不会发生」的数据：一个 zone 漏掉一条热区，
 * 那条术语在分区态里就永远出不来，而页面不会报任何错。所以全部在构建期挡住。
 * 成员数硬限 3–7 是行业解剖图的标注密度区间（research/03 §4.1）。 */
function validateStageZones(manifest, claims, errors) {
  const stageId = manifest.id
  const claimBySlot = new Map(claims.map((claim) => [claim.termId, claim.slot]))
  const zones = manifest.zones
  if (!Array.isArray(zones) || !zones.length) {
    errors.push(`舞台 ${stageId} 没有声明 zones`)
    return
  }

  const SLOT_FIELD = { hotspot: 'hotspotIds', variant: 'variantIds', param: 'paramIds' }
  const zoneIds = new Set()
  const ownerOf = new Map()

  for (const zone of zones) {
    if (!zone?.id) { errors.push(`舞台 ${stageId} 有一个分区缺少 id`); continue }
    if (zoneIds.has(zone.id)) errors.push(`舞台 ${stageId} 的分区 id 重复：${zone.id}`)
    zoneIds.add(zone.id)
    if (!zone.labelZh) errors.push(`舞台 ${stageId} 的分区 ${zone.id} 缺少 labelZh`)
    if (!zone.descriptionZh) errors.push(`舞台 ${stageId} 的分区 ${zone.id} 缺少 descriptionZh`)

    const members = zoneMemberIds(zone)
    if (members.length < ZONE_MIN_MEMBERS || members.length > ZONE_MAX_MEMBERS) {
      errors.push(
        `舞台 ${stageId} 的分区 ${zone.id} 有 ${members.length} 个成员，超出 ${ZONE_MIN_MEMBERS}–${ZONE_MAX_MEMBERS} 的区间`,
      )
    }
    for (const slot of SLOTS) {
      for (const termId of zone[SLOT_FIELD[slot]] || []) {
        const actual = claimBySlot.get(termId)
        if (!actual) {
          errors.push(`舞台 ${stageId} 的分区 ${zone.id} 收了本台没有认领的术语：${termId}`)
          continue
        }
        if (actual !== slot) {
          errors.push(`舞台 ${stageId} 的分区 ${zone.id} 把 ${slot} 位的 ${termId} 写成了 ${SLOT_LABEL[actual]}（应放进 ${SLOT_FIELD[actual]}）`)
        }
        if (ownerOf.has(termId)) {
          errors.push(`舞台 ${stageId} 的 ${termId} 同时属于两个分区：${ownerOf.get(termId)} 与 ${zone.id}`)
          continue
        }
        ownerOf.set(termId, zone.id)
      }
    }
  }

  for (const claim of claims) {
    if (!ownerOf.has(claim.termId)) {
      errors.push(`舞台 ${stageId} 的 ${claim.termId} 没有被任何分区覆盖`)
    }
  }

  /* ---- 对照组 ---- */
  const axisIds = new Set(COMPARE_AXES.map((axis) => axis.id))
  const setIds = new Set()
  for (const set of manifest.compareSets || []) {
    if (!set?.id) { errors.push(`舞台 ${stageId} 有一个对照组缺少 id`); continue }
    if (setIds.has(set.id)) errors.push(`舞台 ${stageId} 的对照组 id 重复：${set.id}`)
    setIds.add(set.id)
    if (!set.titleZh) errors.push(`舞台 ${stageId} 的对照组 ${set.id} 缺少 titleZh`)
    if (set.zoneId && !zoneIds.has(set.zoneId)) {
      errors.push(`舞台 ${stageId} 的对照组 ${set.id} 指向不存在的分区：${set.zoneId}`)
    }
    const termIds = set.termIds || []
    if (termIds.length < 2) errors.push(`舞台 ${stageId} 的对照组 ${set.id} 至少要有两条术语`)
    for (const termId of termIds) {
      if (!claimBySlot.has(termId)) {
        errors.push(`舞台 ${stageId} 的对照组 ${set.id} 引用了不在本台的术语：${termId}`)
      }
    }
    for (const axis of set.axes || []) {
      if (!axisIds.has(axis?.id)) errors.push(`舞台 ${stageId} 的对照组 ${set.id} 使用了未知的轴：${axis?.id}`)
    }
    for (const [termId, row] of Object.entries(set.cells || {})) {
      if (!termIds.includes(termId)) {
        errors.push(`舞台 ${stageId} 的对照组 ${set.id} 给不在组内的 ${termId} 写了格子`)
        continue
      }
      for (const axisId of Object.keys(row || {})) {
        if (!axisIds.has(axisId)) errors.push(`舞台 ${stageId} 的对照组 ${set.id} 的 ${termId} 写了未知的轴：${axisId}`)
      }
    }
  }
  ...
  for (const entry of manifest.positionRegions || []) {
    if (!POSITION_REGION_IDS.includes(entry?.region)) {
      errors.push(`舞台 ${stageId} 声明了未知的页面位置区域：${entry?.region}`)
      continue
    }
    if (seenRegions.has(entry.region)) errors.push(`舞台 ${stageId} 重复声明位置区域：${entry.region}`)
    seenRegions.add(entry.region)
    for (const termId of entry.termIds || []) {
      if (!claimBySlot.has(termId)) {
        errors.push(`舞台 ${stageId} 的位置区域 ${entry.region} 收了本台没有认领的术语：${termId}`)
      }
    }
  }
}
...
export function buildStageIndex(manifests, atlas, { strict = true } = {}) {
  ...
  const stages = manifests.map((manifest) => {
    ...
    validateStageZones(manifest, claims, errors)
    ...
    return {
      ...
      claims,
      groups,
      /* 分区 / 对照组 / 位置区域原样透出，供图鉴外壳（WP-F）渲染。 */
      zones: manifest.zones || [],
      compareSets: manifest.compareSets || [],
      positionRegions: manifest.positionRegions || [],
    }
  })
