...
import { QUERY_ALIASES } from '../data/query-aliases.js'
import { extractQuerySignals, scoreSemanticCandidate } from './query-understanding.js'
 ...
 */
function rankRecords(records, keyword) {
  const kw = normalize(keyword)
  if (!kw) return { records: [], uncertain: false, signals: [] }
  const signals = extractQuerySignals(kw, QUERY_ALIASES)
  const hits = asArray(records)
    .map((record) => {
      const semantic = scoreSemanticCandidate(record, signals)
      const lexicalScore = record.text.includes(kw) ? scoreOf(record, kw) : 0
      if (lexicalScore === 0 && semantic.score === 0) return null
      return {
        record: semantic.reasons.length
          ? { ...record, matchReasons: semantic.reasons }
          : record,
        score: lexicalScore + semantic.score,
      }
    })
    .filter(Boolean)
  ...
  const uncertain = signals.length > 0 && best > 0 && hits.filter((hit) => hit.score === best).length > 1
  return {
...
export function searchRecords(records, keyword, limit) {
  const ranked = rankRecords(records, keyword)
  return Number.isFinite(limit) ? ranked.records.slice(0, limit) : ranked.records
}

/** 两组结果 + 各自的总命中数（「查看全部」要显示的就是这个总数）。 */
export function searchAll(index, keyword, limit) {
  const termRank = rankRecords(index?.terms, keyword)
  const siteRank = rankRecords(index?.sites, keyword)
  const terms = termRank.records
  const sites = siteRank.records
  return {
    ...
    termUncertain: termRank.uncertain,
    siteUncertain: siteRank.uncertain,
    signals: [...termRank.signals, ...siteRank.signals],
  }
}
