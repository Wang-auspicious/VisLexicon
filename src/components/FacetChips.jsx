...
export default function FacetChips({ items, selections, onToggle, onClear }) {
  ...
  const panelId = useId()
  /* chip 的计数分母永远是**全集**，所以它只跟语料变，不跟筛选变。把它 memo 掉：
   * 近万条语料下，这里每条轴都要扫一遍全集，五次扫描乘以每次敲键就会显出来。
   * selections 变了只是重算「选中态」，那是几十次比较的事，没必要连计数一起重来。 */
  const groups = useMemo(
    () => chipAxes(items, selections, locale),
    [items, selections, locale],
  )
