...
/* 用 memo 包一层。
 *
 * 卡片的两个 prop（`item` 对象引用、`priority` 布尔）在筛选变化时都不变——变的是
 * 「哪些卡片被渲染」，不是「同一张卡片的内容」。近万条语料下，用户每敲一个搜索字符
 * 都会重渲染当前这一屏，而每张卡都带状态和一次收藏读取，白白重算一遍就是几百毫秒。
 * 这条只影响重渲染，不动任何标记与样式。 */
function SiteCard({ item, priority = false }) {
  const t = useT()
  ...
  useEffect(() => {
    ...
    repo.getFavorite(entryId).then((value) => setFavorite(Boolean(value)))
    return undefined
  }, [entryId, repo])
  if (!item) return null
  const href = `#/site/${entryId}`
  const aspect = aspectFor(item)
  const toggleFavorite = async () => { if (favorite) await repo.removeFavorite(entryId); else await repo.upsertFavorite({ entryId }); setFavorite(!favorite) }
  ...
  return (
    <article className="vl-card vl-card--grid" data-entry-id={entryId} data-template={templateFor(item)}>
