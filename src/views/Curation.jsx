...
import {
  HEAT_METHOD_EN,
  HEAT_METHOD_ZH,
  recommendationReasonOf,
  selectCuratorPicks,
  publishedItems,
  selectHeatRanked,
  selectRecentlyChecked,
} from '../lib/curation-selectors.js'

/* ============ 策展首页 ============
 * 策展就是罗列。首页 = 承诺句 + 一行真实数字 + 若干编辑分组的卡片墙。
 * 每组：标题 + 一句说明 + 一排卡片。卡片就是全站同一种卡。
...
export default function Curation() {
  ...
  const items = useMemo(() => publishedItems(state.index?.items ?? []), [state.index])
  ...
  const discoveryRows = useMemo(() => {
    const rows = []
    /* 热门排行：口径见 curation-selectors 的 HEAT_METHOD_*。没有服务器聚合的访问量时
     * 不编数字，改用四项本地可复算的收录信号；口径挂在标题上供核对。 */
    const heat = selectHeatRanked(items, COLLECTIONS, 6)
    const recentlyChecked = selectRecentlyChecked(items, 6)
    const recommendations = selectCuratorPicks(items, 6)
    const labels = locale === 'en'
      ? { trending: 'Trending', checked: 'Recently verified', recommended: 'Curator picks' }
      : { trending: '热门排行', checked: '最近核验', recommended: '站长推荐' }
    const heatNote = locale === 'en' ? HEAT_METHOD_EN : HEAT_METHOD_ZH
    if (heat.length) rows.push({ key: 'trending', title: labels.trending, note: heatNote, items: heat })
    if (recentlyChecked.length) rows.push({ key: 'checked', title: labels.checked, items: recentlyChecked })
    if (recommendations.length) rows.push({ key: 'recommended', title: labels.recommended, items: recommendations })
    return rows
  }, [items, locale])
  ...
  return (
    <div className="vl-curation">
      ...
      {state.status === 'ready' ? <section className="vl-discovery-rows" aria-label={locale === 'en' ? 'Recently verified and curator picks' : '最近核验与站长推荐'}>
        {discoveryRows.map((row) => row.items.length ? <div className="vl-discovery-row" key={row.key}><header><h2 title={row.note ?? undefined}>{row.title}</h2><a href="#/sites">{locale === 'en' ? 'View all →' : '查看全部 →'}</a></header><div>{row.items.map((item, index) => <a className="vl-discovery-item" href={`#/site/${item.entryId}`} key={item.entryId} title={recommendationReasonOf(item) || undefined}><b>{String(index + 1).padStart(2, '0')}</b><span>{item.name}</span></a>)}</div></div> : null)}
      </section> : null}
