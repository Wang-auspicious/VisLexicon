...
import { loadPublicSiteDetail } from '../lib/public-data.js'
import { useT } from '../i18n.js'

/**
 * 站点详情：`#/site/<entryId>` 的路由页。
 * 桌面浮窗、手机底部抽屉，由 CSS 在 768px 处切换。
 * 阅读顺序：身份 → 简介与元数据 → 三图。点遮罩关闭。
 */
...
export default function SiteDetail({ entryId, onClose }) {
  ...
  useEffect(() => {
    ...
    loadPublicSiteDetail(entryId, { signal: controller.signal })
      .then((data) => ({ status: 'ready', data }))
      .then(setState)
      .catch((error) => {
        if (error?.name === 'AbortError') return
        if (error?.status === 404 || error?.code === 'MISSING_PUBLIC_RECORD') {
