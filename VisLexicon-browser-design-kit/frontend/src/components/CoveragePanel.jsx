...
export default function CoveragePanel({ data }) {
  ...
  const names = Array.isArray(catalogOverride) && catalogOverride.length > 0
    ? catalogOverride
    : (live?.names || [])
  const shown = open ? names : names.slice(0, PREVIEW)
  const overflow = names.length > PREVIEW
  const acquire = voice?.acquire?.[locale] || []
  const style = voiceText(data.entryId, 'styleDetail', locale)
    || voiceText(data.entryId, 'style', locale)
      ...
      {names.length > 0 ? (
        <div className="sd-cover-block">
          ...
          <p className="sd-cover-names">
            {shown.map((name) => (
              <span className="sd-pill sd-pill-comp" key={name}>{name}</span>
            ))}
            {overflow ? (
              <button
                type="button"
