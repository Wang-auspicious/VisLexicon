    ...
    <div className="pg" style={{ '--pg-size': `${tileSize}px`, '--pg-radius': `${radius}px` }}>
      <div className="pg-main">
        ...
        >
          ...
          <div {...node('gesture.dropzone', 'pg-dropzone')}>放置区</div>
        </div>

        <div className="pg-side">
          <div {...node('gesture.press', 'pg-press')} data-state={phase}>
            <i />按压反馈
          </div>
          <button type="button" {...node('gesture.focusring', 'pg-focusring')}>焦点环 · Tab 试试</button>
          <div {...node('gesture.dismiss', `pg-dismiss ${dismissed ? 'gone' : ''}`)}>
            {dismissed ? '已滑走 · 重演可恢复' : '横向快滑我'}
          </div>
          <div {...node('gesture.reorder', 'pg-reorder')}>
